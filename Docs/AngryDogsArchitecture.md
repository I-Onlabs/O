...
2. **Refactor Player Flow**
   - Split the prototype `PlayerController` into:
     - `PlayerController` (high-level orchestrator, event routing).
     - `PlayerMovementController` (movement via `CharacterController` or root-motion).
     - `PlayerShooter` (fire rate, projectile pooling, obstacle conversion).
     - `NibbleCompanionController` (pet behaviours, buff triggers).
   - Use dependency injection through serialized references or service locator access.

3. **Enhance Input Handling**
   - Introduce `PlayerInputHandler` supporting Unity Input System (actions asset) with graceful fallback to legacy `Input` APIs for mobile builds.
   - Provide smoothing for swipe/drag and aim reticle assistance.
...
6. **Persistence**
   - Implement `SaveSystem` storing JSON in `Application.persistentDataPath`. Encrypt/obfuscate if necessary. Maintain versioning in `PlayerProgressData`.
...
- [ ] Migrate input to `PlayerInputHandler` with smoothing.
- [ ] Pool projectiles, obstacles, hounds.
- [ ] Implement `SaveSystem` with versioned JSON.
